...
- Phases, in the order they win (`bar/Body.qml`): **stemme** (dictation, modal) → **aabner** (launcher, has the keyboard) → **aaben** (menu) → **besked** (notification, self-retracting unless critical) → **niveau** (vol/light OSD) → **kig** (hover, passive) → **hvile** (battery + clock only).
...
- **Notifications: the pill replaced dunst 2026-08-28** (`services/Notifs.qml`, `NotificationServer`). dunst is out of `hyprland.lua`'s autostart — only one process can own `org.freedesktop.Notifications`, and whoever gets there first wins. Roll back: `pkill qs; dunst &`.
  - Two things, deliberately not one: the **popup** (in the pill, retracts after `body.notifyLinger`, held while the mouse is on it, critical ones never retract) and the **list** (stays until dismissed; surfaces as `N beskeder` on the alert pill). dunst conflated them, so a bubble you missed was simply gone.
...
- Not done yet: the perf menu has no click target (only `perf-mode menu` from a terminal); clock click for a calendar; the power menu (`bye`) as a pill state. Next steps live in `~/.claude/plans/hele-shellen-til-quickshell.md`.
