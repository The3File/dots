...
- **Rule for this shell: no flade without an IPC entrance. If Claude can't call it, it isn't finished.** The shell is agent-*driven*, not agent-*like* — it grows IPC, not natural language.
  ...
  - `sudo spoerg <argv>|kode <argv>|svar|afbryd|state` — root-access asked in the pill (`services/Sudo.qml`). Two calls on purpose: raise returns immediately, `svar` is polled and gives `venter` until answered. An IPC call that waited for a human would block Quickshell's thread the whole time. Answers: `ja` / `nej` / `ja:<password>`; the pill closing without an answer is a `nej`, so a question that disappears can never let a command through.
...
| Claude Code / Cursor sudo | Agents must use `~/.Scripts/claude-sudo-run` (hook rewrites bare `sudo`). **The question is asked in the pill** (`pill-sudo` → IPC `sudo`), showing the **exact argv**; then PTY `sudo -A -k` (fingerprint → password field in the pill via `pill-askpass`). Zenity is the fallback for both halves and stays untouched — `pill-sudo` exits 2 when the shell is down, and the old flow runs. Askpass reads `SUDO_DISPLAY_CMD` — plain `sudo -A` askpass does **not** get `SUDO_COMMAND` (always empty), which is why dialogs used to say only “sudo”. Cursor also injects `cursor-askpass` + `alias sudo='sudo -A'`; the hook bypasses that. Do not call `/usr/bin/sudo -A` with `zenity-askpass-password` from agents. |
