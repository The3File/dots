...
- **Kanten.** Pillerne bruger `Theme.pillBorder` (= wal `color7`/foreground, paletten's lyseste tone — mod den moerke pille laeses den som hvid, og den foelger tapetet i stedet for at vaere et fast `#fff`) og `Config.borderWidth` (= `border_size` i `hyprland.lua`, 2). Tykkelsen matcher vinduerne; **farven goer det med vilje ikke** — Hyprland tegner `col.active_border` med `color4`, som er moerkere. Farven foelger paletten af sig selv; **tykkelsen er en kopi** — aendres `border_size`, skal `borderWidth` med. Skal vinduerne en dag vaere lyse ogsaa, er det `chwal` der saar `~/.config/hypr/colors.lua`, ikke en haandrettelse i den fil. Kun afvigelser faar en anden kantfarve: stemmen, root-spoergsmaalet, en besked. Aabneren og menuen har ikke laengere deres egen.
