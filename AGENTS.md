...
- **Kanten.** Pillerne bruger `Theme.pillBorder` (**fast hvid, ikke fra paletten** — samme valg som den sorte `barBackground`: paletten kan lande hvor som helst, og kanten skal vaere lys hver gang) og `Config.borderWidth` (= `border_size` i `hyprland.lua`, 2). Tykkelsen matcher vinduerne; **farven goer det med vilje ikke** — Hyprland tegner `col.active_border` med wal `color4`, som `chwal` saar ind i `~/.config/hypr/colors.lua` ved hvert tapetskift (den fil skal derfor aldrig rettes i haanden — ret `apply_hyprland()` i `chwal`). **Tykkelsen er en kopi** — aendres `border_size`, skal `borderWidth` med. Kun afvigelser faar en anden kantfarve: stemmen, root-spoergsmaalet, en besked. Aabneren og menuen har ikke laengere deres egen.
