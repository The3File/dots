...
| hyprwhspr busy boost | During dictation, `hyprwhspr-perf` temporarily forces EPP/`platform_profile`=`performance`, then `tlp bat`/`tlp ac` restores the profile above |
...
| Piece | Detail |
|-------|--------|
| sudo | `%wheel` requires auth — **no NOPASSWD**; PAM `pam_fprintd.so` (sufficient) + password via `system-auth` (`/etc/pam.d/sudo`) |
...
- Touchpad: 3-finger horizontal swipe changes workspace; 3-finger down opens AIOS (`scratchaios`), up closes it (`hl.gesture` in `hyprland.lua`).
