...
| Bar (Quickshell) | `~/.config/quickshell/ringdal/` (+ `config.json`) |
...
- Hyprland stand-in for the old X11 lemonbar: **bottom**. Modules: workspaces, perf, hyprwhspr, **keylock** (`kbd: enabled|disabled`, click toggles), koffein, vol/light/net/bat/clock.
- **Quickshell** (QtQuick/QML) replaced waybar 2026-08-27. Config: `~/.config/quickshell/ringdal/` — `shell.qml`, `Config.qml`, `Theme.qml`, `services/`, `widgets/`, `bar/`. Started by `hyprland.lua` as `qs -c ringdal`. Verified pixel-identical to the old waybar across the full width before the switch.
...
- Net click → `~/.Scripts/fuzzel_nm` (WiFi/connections). Bluetooth: Super+Shift+B → `~/.Scripts/btcon`. ThinkPad LEDs: `pwrbtnlght` (fuzzel menu / CLI; video group, no /home/ringdal/.Scripts/claude-sudo-run). Micmute LED inverted via `pwrbtnlght sync|watch` (muted=on); power LED forced off. One-shot: `/home/ringdal/.Scripts/claude-sudo-run ~/.Scripts/thinkpad-leds-apply` (udev `trigger=none`).
...
- Not done yet: popup horizontal placement sits beside the module rather than above it (cosmetic; the popup machinery itself works). Next steps live in `~/.claude/plans/lad-os-skifte-waybar-clever-pebble.md` — perf/net menus as bar popups instead of fuzzel, vol/light OSD, `Services.Notifications` to replace dunst, launcher to replace `fuzzel_drun`.
...
- Bluetooth: **btcon** — Super+Shift+B (fuzzel; CLI: `btcon list|saved|scan|connect <navn>|disconnect <navn|all>|remove <navn|all>|forget <navn|all>|power`)
...
- WiFi / NM: **fuzzel_nm** — Super+Shift+W (also waybar net click)
