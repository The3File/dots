...
- Phases, in the order they win (`bar/Body.qml`): **stemme** (dictation, modal) → **aabner** (launcher, has the keyboard) → **aaben** (menu) → **niveau** (vol/light OSD) → **kig** (hover, passive) → **agent** (Claude working — see below) → **hvile** (battery + clock only).
...
- **Agentfladen (`services/Agent.qml`, `bar/AgentContent.qml`) — 2026-08-28.** What Claude is doing *while it runs*, not a bubble after the fact. Three states: `ledig` (nothing shown), `arbejder` (a breathing dot + the latest line, standing in for battery/clock in the body — it *is* the action in progress), `venter` (`venter` on the alert pill so an open menu or dictation can't hide it, plus the line in the body).
  - The lines come from **`tale`** (`AIOS/tale/tale`) — the same short sentences he hears. The line is the information; voice and pill are two ways of delivering it, so `tale` pushes to the pill **before** its `er_til` check and the visual stream works with speech switched off. `qs` missing (Termux, Mac) = silent no-op.
  - Hooks need no wiring: `tale __tur` (UserPromptSubmit) → `working`, `tale __net` (Stop) → `done`, `tale __varsel` (Notification) → `say dig`. Level `dig` means *needs him now*, so it sets `venter` rather than being a mere colour.
  - The `notify-send` lines for 'Claude er færdig' / 'Claude venter' were **removed from `~/.claude/settings.json`** at the same time (backup: `settings.json.bak.pre-agentflade`) — the pill says it better, and since notifications now persist until dismissed, one bubble per turn would silently fill the list.
  - IPC: `agent working|waiting|done|state`, `agent say <frem|obs|dig> <text>`.
