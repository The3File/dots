...
| sudo-pty gotcha | `sudo -k -v` **deliberately refuses to save the ticket it just earned** — the finger is accepted and the next `sudo -n` still says “a password is required”. Use `sudo -K` as its own call first (throws the old ticket away, asks nothing), then `sudo -A -v`. Both sudo calls must share one PTY: tickets are per-tty by default. `pty.spawn` (not `pty.fork`) so stdin is still forwarded — `echo x \| sudo tee f` has to keep working. The “finger accepted” signal is an OSC escape (`\e]777;sudo-ok\a`) filtered out of the stream: invisible if it ever slips through, unlike a word. |
