...
- **Kanten er vinduernes kant.** Pillerne bruger `Theme.windowBorder` (= wal `color4`, samme farve Hyprland tegner `col.active_border` med via `~/.config/hypr/colors.lua`) og `Config.borderWidth` (= `border_size` i `hyprland.lua`, 2). Farven foelger paletten af sig selv; **tykkelsen er en kopi** — aendres `border_size`, skal `borderWidth` med. Kun afvigelser faar en anden kantfarve: stemmen, root-spoergsmaalet, en besked. Aabneren og menuen har ikke laengere deres egen — den var color4, som nu er grundfarven.
