...
- Both scripts gained a non-interactive CLI for this (their fuzzel menus are untouched and remain the fallback): `fuzzel_nm list|ssids|up|down|connect <ssid> [pass]|delete`, and `btcon` additionally `trust|untrust|block|unblock|pair <target>` + `power on|off|toggle`.
...
- Not done yet: the perf menu has no click target (only `perf-mode menu` from a terminal); the clipboard (`fuzzel_clip`, Super+Insert); clock click for a calendar; the power menu (`bye`) as a pill state. Next steps live in `~/.claude/plans/hele-shellen-til-quickshell.md`.
