...
FSA = {
  ...
  getName: function(e, format){
    ...
    if (format) {
      ...
      oName = format.replace('$i', index).replace('$pi', padIndex)
        .replace('$o', oName)
        .replace('$dc', date.replace(/-/g, '')).replace('$d', date)
  ...
  getHTML: function(mode){
    ...
    $('script')[0].src = 'https://rawgit.com/inDream/DownAlbum/master/assets/jquery.min.js';
    $('link:eq(0)').prop('href', 'https://rawgit.com/inDream/DownAlbum/master/assets/bootstrap.min.css');
    $('script:gt(0)').remove();
...
Core = {
  ...
  loadData: function (c) {
    ...
    $('#aDes').html(c.aDes);
    $('#aTime').html(c.aTime);
    $('#dTime').html('Download at: ' + c.dTime);
    var d = c.photos;
    if(!d.length)return;
    ...
    for (var i = 0; i < d.length; i++) {
      ...
      var TEMPLATE = "";
      if(c.newLayout){
        var srcInd = href.length ? '<a ' + href + '><i class="tagArrow srcInd"></i></a>' : '';
        TEMPLATE = '<div class="col-4 col-lg-3"><div id="item' + i + '" class="thumbnail item' + (c.largeAlbum?' largeAlbum':'') + '" rel="gallery"><div class="tools">' + srcInd + videoInd + dateInd + '<span class="date">' + d[i].date + '</span><a class="selector"><span class="badge">' + (i+1) + '</span></a>' + commentInd + tagIndi + '</div><a class="fancybox" rel="fancybox" href="' + d[i].url + '" target="_blank"><div class="crop"><div style="background-image: url(' + d[i].url + ');" class="img"><img src="' + d[i].url + '"></div></div></a>' + title + tag + comments + '</a></div></div>';
      }else{
        TEMPLATE = '<div rel="gallery" class="item' + (c.largeAlbum?' largeAlbum':'') + '" id="item' + i + '"><a class="badge"' + href + '>' + (i+1) + '</a>' + commentInd + tagIndi + videoInd + dateInd + '<a class="fancybox" rel="fancybox" href="' + d[i].url + '" target="_blank"><div class="crop"><div style="background-image: url(' + d[i].url + ');" class="img"><img src="' + d[i].url + '"></div></div></a>' + title + tag + comments + '</div>'
      }
      b = b + "" + TEMPLATE;
    }
  ...
  initEvent: function(){
    ...
    $('script:gt(0)').remove();
    $('script').attr('src','https://rawgit.com/inDream/DownAlbum/master/assets/jquery.min.js');
    $("link:eq(0)").prop("href", "https://rawgit.com/inDream/DownAlbum/master/assets/bootstrap.min.css");
  ...
  albumProcess: function(){
    ...
    if(entry)entry.file(function(file){
      ...
      reader.onload = function(e){
        ...
        if (html.indexOf('4013937') > 0 || html.indexOf('DownAlbum') > 0) {
          html = $(html);
          //FSA.temp = html;
