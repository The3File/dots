...
var k = document.querySelectorAll('button'), loadCm = true, largeAlbum;
...
chrome.runtime.getBackgroundPage(function(w){
  ...
  document.querySelector('#patreon').addEventListener('click', function(){
    chrome.tabs.create({url: 'http://goo.gl/mHHTjp'});
    ga('send', 'event', 'button', 'click', 'popup: Patreon');
    window.close();
  });
...
function exeScript(mode){
  chrome.tabs.query({active: true}, function(tab){
    ...
    chrome.tabs.executeScript(tab.id, {file: "./js/md5.min.js"});
