...
function msgHandle(msg, sender, sendResponse) {
  switch(msg.type){
    ...
    case 'remapAlbum':
      ...
      break;
    case 'getIgUserInfo':
      fetch(`https://i.instagram.com/api/v1/users/${msg.id}/info/`)
        .then(r => r.json())
        .then(r => sendResponse({
          id: msg.id,
      ...
      return true;
    case 'getWeiboAlbumList':
      fetch(`https://photo.weibo.com/albums/get_all?uid=${msg.uId}&page=1&count=20`)
        .then(r => r.json())
        .then(r => sendResponse(r));
      return true;
    case 'getWeiboAlbum':
      fetch(`https://photo.weibo.com/photos/get_all?uid=${msg.uId}&` +
        `album_id=${msg.aId}&count=30&page=${msg.ajaxPage}&type=3`)
        .then(r => r.json())
        .then(r => sendResponse(r));
      return true;
    case 'getTwitter':
      fetch('https://api.twitter.com/2/timeline/media/' + msg.id +
        '.json?skip_status=1&tweet_mode=extended&include_entities=false&count=20' +
        (msg.ajax ? ('&cursor=' + encodeURIComponent(msg.ajax)) : ''), {
          credentials: 'include',
          headers: {
...
function setRequestListener(){
  ...
  const storyBase = 'https://i.instagram.com/';
  chrome.webRequest.onBeforeSendHeaders.addListener((details) => {
    const headers = [];
    details.requestHeaders.forEach((header) => {
      const headerName = header.name.toLowerCase();
      if (headerName.indexOf('x-devtools') === 0) {
        return;
      }
      if (headerName === 'user-agent') {
        header.value = phoneUA;
      }
      headers.push(header);
    });
    headers.push({ name: 'Referer', value: base });
    headers.push({ name: 'x-ig-capabilities', value: '3brTPw==' });
    return { requestHeaders: headers };
  }, { urls: [`${storyBase}*`] }, ['blocking', 'requestHeaders']);
}
...
function checkForValidUrl(tabId, changeInfo, tab) {
  ...
  var regx = new RegExp(extUrl + '|facebook.com|instagram.com|ask.fm|' +
    'pinterest|twitter.com|weibo.com', 'i');
  currentTabId = tabId;
